...
static void scroll() {
  ...
  // 值为：0000 1111 0010 0000， 其中前面8位是属性，后面8位是ASCII码
  ...
  if (cursor_y >= 25) {
    ...
    int i;
    for (i = 0 * 80; i < 24 * 80; i++) {  // 分辨率默认80*25
...
void console_putc_color(char c, real_color_t back, real_color_t fore) {
  ...
  uint8_t attribute_byte =
      (back_color << 4) | (fore_color & 0x0F);  // 需要把fore_color的前4位置0
...
// 屏幕打印一个以 \0 结尾的字符串(默认黑底白字)，\0的ASCII码为0
